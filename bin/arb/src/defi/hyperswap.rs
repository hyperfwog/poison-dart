...
/// HyperSwap DEX implementation (Uniswap V2 fork)
#[derive(Clone)]
pub struct HyperSwap {
    pool: Pool,
    pool_arg: ObjectArg,
    liquidity: u128,
    coin_in_type: String,
...
impl HyperSwap {
    pub async fn new(simulator: Arc<Box<dyn Simulator>>, pool: &Pool, coin_in_type: &str) -> Result<Self> {
        ensure!(pool.protocol == Protocol::HyperSwapV2, "not a HyperSwap pool");
    ...
    fn build_swap_args(&self, ctx: &mut TradeCtx, coin_in_arg: Argument) -> Result<Vec<Argument>> {
        ...
        // Arguments for swap function: pool, coin_in, recipient
        Ok(vec![pool_arg, coin_in_arg])
    }
}

#[async_trait::async_trait]
impl Dex for HyperSwap {
    async fn extend_trade_tx(
        ...
        let function = if self.is_a2b() { "swap_exact_input_0_for_1" } else { "swap_exact_input_1_for_0" };

        // HyperSwap package ID
        let package = ObjectID::from_hex_literal("0x724412C00059bf7d6ee7d4a1d0D5cd4de3ea1C48")?;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_hyperswap_swap_tx() {
        ...
        let token_out_type = "0x50c42dEAcD8Fc9773493ED674b675bE577f2634b::coin::WETH";
        let amount_in = 10000;
        ...
        let dex = dexes
            .into_iter()
            .filter(|dex| dex.protocol() == Protocol::HyperSwapV2)
