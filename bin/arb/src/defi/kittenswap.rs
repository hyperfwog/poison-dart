...
impl KittenSwap {
    pub async fn new(simulator: Arc<Box<dyn Simulator>>, pool: &Pool, coin_in_type: &str) -> Result<Self> {
            ...
            pool.protocol == Protocol::KittenSwap || pool.protocol == Protocol::KittenSwapStable,
            "not a KittenSwap pool"
        );
        ...
        // Extract liquidity from pool object
        let liquidity = {
            // For simplicity, we'll use a placeholder value
            // In a real implementation, you would extract the reserves from the pool object
        ...
        let type_params = vec![
            TypeTag::from_str(&pool.token0_type())?,
            TypeTag::from_str(&pool.token1_type())?,
        ];

        let pool_arg = shared_obj_arg(&pool_obj, true);
        let is_stable = pool.protocol == Protocol::KittenSwapStable;
    ...
    fn build_swap_args(&self, ctx: &mut TradeCtx, coin_in_arg: Argument) -> Result<Vec<Argument>> {
        ...
        // Arguments for swap function: pool, coin_in, stable flag
        let stable_arg = ctx.pure(self.is_stable)?;
        Ok(vec![pool_arg, coin_in_arg, stable_arg])
    }
}
...
impl Dex for KittenSwap {
    async fn extend_trade_tx(
        ...
        // Determine which swap function to call based on token order
        let function = if self.is_a2b() {
            "swap_exact_tokens_for_tokens_simple_a_to_b"
        } else {
            "swap_exact_tokens_for_tokens_simple_b_to_a"
        };

        // KittenSwap package ID
        let package = ObjectID::from_hex_literal("0xDa12F450580A4cc485C3b501BAB7b0B3cbc3B31B")?;
        let module = Identifier::new("router").map_err(|e| eyre!(e))?;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_kittenswap_swap_tx() {
        ...
        let token_out_type = "0x29219dd400f2Bf60E5a23d13Be72B486D4038894::coin::USDC";
        let amount_in = 10000;
        ...
        let dex = dexes
            .into_iter()
            .filter(|dex| dex.protocol() == Protocol::KittenSwap || dex.protocol() == Protocol::KittenSwapStable)
